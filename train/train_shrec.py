...
from segmesh.utils import ShrecAugment as augment_fn
...
class BaseTrainer:
    ...
    def setup_logging_and_checkpoint(self) -> None:
        ...
        self.write_folder = f'runs_shapes/shrec_CAD_{timestamp}'  # Updated for shrec
    ...
    def backup_training_procedure(self) -> None:
        ...
        os.system(f'cp picasso/networks/shape_cls.py {self.write_folder}')  # Updated for shrec
...
class DatasetHandler:
    ...
    def setup_data(self) -> None:
        """Set up data loaders for the shrec dataset."""
        Meshes, Labels = {'train':[], 'test':[]}, {'train':[], 'test':[]}
        classnames = self.config['classnames']
        for clsname in classnames:
            train_list = glob(f"{self.config['data_dir']}/{clsname}/train/*.obj")
            Meshes['train'] += train_list
            Labels['train'] += [f"{self.config['data_dir']}/{clsname}/label.txt"] * len(train_list)

            test_list = glob(f"{self.config['data_dir']}/{clsname}/test/*.obj")
            Meshes['test'] += test_list
            Labels['test'] += [f"{self.config['data_dir']}/{clsname}/label.txt"] * len(test_list)

        repeat = self.config['batch_size'] * 500 // len(Meshes['train']) + 1
        trainSet = ShapeDataset(Labels['train'] * repeat, Meshes['train'] * repeat,
                                     transform=augment_fn(prob=0.5), normalize=normalize_fn())
        self.train_loader = DataLoader(trainSet, batch_size=self.config['batch_size'], shuffle=True, num_workers=3,
                                       collate_fn=collate_fn())
        
        testSet = ShapeDataset(Labels['test'], Meshes['test'], transform=None, normalize=normalize_fn())
        self.test_loader = DataLoader(testSet, batch_size=1, shuffle=False, num_workers=0,
                                      collate_fn=collate_fn())
...
def main():
    """Main function to set up and start training for the shrec dataset."""
    # Load configuration from YAML file
    config_loader = ConfigLoader("./config/shrec.yaml")
    ...
    # Start training
    logger.info("Starting training for the shrec dataset...")
    trainer.train()
