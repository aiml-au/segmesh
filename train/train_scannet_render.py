...
from segmesh.utils import ScannetAugment as augment_fn
from segmesh.utils import ScannetAlign as align_fn
...
class BaseTrainer:
    ...
    def setup_logging_and_checkpoint(self) -> None:
        ...
        self.write_folder = f'runs_scenes/scannet_render_{timestamp}'
        if 'ckpt_path' in self.config and self.config['ckpt_path']:
    ...
    def backup_training_procedure(self) -> None:
        ...
        os.system(f'cp picasso/networks/scene_seg.py {self.write_folder}')
...
class DatasetHandler:
    ...
    def setup_data(self) -> None:
        """Set up data loaders for the scannet dataset."""
        Meshes, Labels = {}, {}
        ...
        Meshes['train'] = [f"{self.config['data_dir']}/train/{filename}.h5" for filename in train_files]
        Meshes['val'] = [f"{self.config['data_dir']}/val/{filename}.h5" for filename in val_files]
        Labels['train'] = [f"{self.config['data_dir']}/train/{filename}.txt" for filename in train_files]
        Labels['val'] = [f"{self.config['data_dir']}/val/{filename}.txt" for filename in val_files]

        repeat = np.ceil((600 * self.config['batch_size']) / len(Meshes['train'])).astype('int32')
        trainSet = SceneDataset(Labels['train'] * repeat, Meshes['train'] * repeat,
                                     transform=augment_fn(0.9), normalize=align_fn())
        self.train_loader = DataLoader(trainSet, shuffle=True, batch_size=1, num_workers=6,
                                       collate_fn=CustomCollateSceneDataset(batch_size=self.config['batch_size'], max_nv=self.config['max_nv']))

        testSet = SceneDataset(Labels['val'], Meshes['val'], transform=None, normalize=align_fn())
        self.test_loader = DataLoader(testSet, shuffle=True, batch_size=1, num_workers=0,
                                      collate_fn=CustomCollateSceneDataset(batch_size=1, max_nv=self.config['max_nv']))
...
def main():
    """Main function to set up and start training for the scannet dataset."""
    # Load configuration from YAML file
    config_loader = ConfigLoader("./config/scannet.yaml")
    ...
    # Start training
    logger.info("Starting training for the scannet dataset...")
    trainer.train()
