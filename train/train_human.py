...
from segmesh.utils import HumanAugment as augment_fn
...
class BaseTrainer:
    ...
    def setup_logging_and_checkpoint(self) -> None:
        ...
        self.write_folder = f'runs_shapes/human_{timestamp}'
    ...
    def backup_training_procedure(self) -> None:
        ...
        # Assuming the layers file is also relevant for the human dataset
...
class DatasetHandler:
    ...
    def setup_data(self) -> None:
        """Set up data loaders for the human dataset."""
        Meshes, Labels = {}, {}
...
def main():
    """Main function to set up and start training for the human dataset."""
    # Load configuration from YAML file
    config_loader = ConfigLoader("./config/human.yaml")
    config = config_loader.load_config()

    # Setup CUDA device
    device = torch.device(f"cuda:{config['gpu']}" if torch.cuda.is_available() else "cpu")

    # Num classes for this dataset
    NUM_CLASSES = len(config['classnames'])

    # Setup model, optimizer, loss function & scheduler
