...
from segmesh.utils import CosegAugment as augment_fn
...
class BaseTrainer:
    ...
    def setup_logging_and_checkpoint(self) -> None:
        ...
        self.write_folder = f'runs_shapes/coseg_{timestamp}'
    ...
    def backup_training_procedure(self) -> None:
        ...
        os.system(f'cp segmesh/networks/shape_seg.py {self.write_folder}')
    ...
    def train(self) -> None:
        ...
        if 'coseg_chairs' in dataset:
            classnames = ['class%d' % i for i in range(3)]
        elif 'coseg_aliens' in dataset or 'coseg_vases' in dataset:
            classnames = ['class%d' % i for i in range(4)]
        else:
...
class DatasetHandler:
    ...
    def setup_data(self) -> None:
        """Set up data loaders for the coseg dataset."""
        dataset = os.path.basename(self.config['data_dir'])
        Meshes, Labels = {}, {}
        train_files = [os.path.basename(file).replace('.obj', '') for file in glob(f"{self.config['data_dir']}/train/*.obj")]
        test_files = [os.path.basename(file).replace('.obj', '') for file in glob(f"{self.config['data_dir']}/test/*.obj")]
        Meshes['train'] = [f"{self.config['data_dir']}/train/{filename}.obj" for filename in train_files]
        Meshes['test'] = [f"{self.config['data_dir']}/test/{filename}.obj" for filename in test_files]
        Labels['train'] = [f"{self.config['data_dir']}/face_label/{filename}.txt" for filename in train_files]
        Labels['test'] = [f"{self.config['data_dir']}/face_label/{filename}.txt" for filename in test_files]

        repeat = self.config['batch_size'] * 500 // len(Meshes['train']) + 1
        trainSet = ShapeDataset(Labels['train'] * repeat, Meshes['train'] * repeat, transform=augment_fn(0.5), normalize=normalize_fn())
        self.train_loader = DataLoader(trainSet, batch_size=self.config['batch_size'], shuffle=True, num_workers=4, collate_fn=collate_fn())
        
        testSet = ShapeDataset(Labels['test'], Meshes['test'], transform=None, normalize=normalize_fn())
        self.test_loader = DataLoader(testSet, batch_size=1, shuffle=False, collate_fn=collate_fn())
...
def main():
    """Main function to set up and start training for the coseg dataset."""
    # Load configuration from YAML file
    config_loader = ConfigLoader("./config/coseg_chairs.yaml")
    config = config_loader.load_config()
    print(os.path.basename(config['data_dir']))
    # Setup CUDA device
    ...
    model = PicassoNetII(num_class=NUM_CLASSES, pred_facet=True, spharm_L=config['degree'], use_height=False).to(device)
    ...
    # Start training
    logger.info("Starting training for the coseg dataset...")
    trainer.train()
